...
| -------- | ------------------ | -------------------------- | ---------- | ---------- |
| 阅读 | [reading.md](../flow/reading.md) | [frontend.md](frontend.md) §页面骨架 | `internal/reader/{handler,service}/` | `features/articles/`、`features/shell/`、`stores/`、`features/settings/`（订阅源来源质量：入板块率 pill + 详情三分解，add-source-board-hit-rate） |
| 偏好画像与订阅源发现 | [discovery.md](../flow/discovery.md) | [backend.md](backend.md) | `internal/admin/`：v1（preference_profile/recommendation/catalog_sync service + handler）+ v2（`discovery_run_service.go` run 化两段执行、`discovery_recall.go` 双路召回、`candidate_{catalog,embedding,check}_service.go` 候选源库/向量回补/可用性、`candidate_catalog_transfer.go` 导入导出、`discovery_v2_switch.go` 等三 ai_settings 配置；handler `discovery_handler.go` 异步受理+runs/:id+interests+history/restore、`candidate_{catalog,access}_handler.go`；scheduler `job_discovery_v2.go` 三 job） | `features/discovery/`（三页签：推荐面板/候选源库/兴趣记录 + run 结果视图 + 四弹窗）、`pages/discovery.vue`、`features/settings/`（兴趣画像 / RSSHub 实例配置 section） |
...
| 数据富化编排 | [data-enrichment.md](../flow/data-enrichment.md) | [backend.md](backend.md) | `internal/dataenrichment/`：双入口——简报 `enrich_board.go`（`EnrichBoard`）+ 调查 `board_investigation_synthesis.go`（`InvestigateBoardQuestion`，配 `board_investigation{,_research}.go`）；另有 `board_brief.go` 简报生成、`situation_cards.go` 态势卡、`freshness_gate.go` 补全门、`analysis_methods.go`+`method_sanitizer.go` 方法卡、`board_brief_review.go`/`board_investigation_review.go` 按 kind 分链 review、`board_interpret.go`+`board_analysis.go`（legacy-only）；handler 侧 `board_qa_handler.go` 板块 QA、`analysis_method_handler.go` 方法卡 CRUD、`analysis_runner.go` job 互斥轮询；跨版块关系发现 `relation_scout.go`+`relation_resolver.go`+`relation_discovery.go`（+`relation_discovery_handler.go` 六路由、`search_internal_context.go` 动态授权） | `features/tags/`(`BoardEnrichmentPanel.vue` 工作台 + 按 result_kind 三分派 `BoardBriefReport.vue` 简报/`BoardInvestigationReport.vue` 调查/`BoardAnalysisReport.vue` legacy、`DebateSection.vue`、`composables/useBoardEnrichment.ts`；跨版块关系 `BoardRelationPanel.vue`+`useBoardRelations.ts`)、设置页「分析方法」section（`SettingsSectionAnalysisMethods.vue`/`AnalysisMethodPanel.vue`） |
...
| 日报 / Digest | [daily-report.md](../flow/daily-report.md) | [tracing.md](tracing.md) | `internal/admin/`(scheduler, daily_report job)、`internal/topicgraph/`(含 lane_snapshot 泳道态势结算 + `GET /semantic-boards/:id/lane-dynamics`) | `features/tags/components/daily-report/`(日报全屏阅读层 + section 可视化)、`features/tags/components/lane-dynamics/`(泳道动态视图)、`features/articles/`(关联文章正文)、`app/utils/{topicAnchor,matchQuality,threadFit}.ts`(observability: System 1 tag↔板块 / System 2 section↔话题 / System 3 thread↔section) |
