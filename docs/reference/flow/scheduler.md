...
- **后端装配**：`backend-go/internal/app/runtime.go`（`registry.Register` 注册所有 scheduler；`resetStaleStates` 之后异步触发 `aihealth.RunStartupProbe`，不阻塞 worker 启动）；关停协议 = 四步序列「HTTP 摘端口(5s) → `Registry.StopAll`(30s) → 工作队列停止(外包 10s) → 耗时汇总行」，摘端口先行、每步有界且 panic 不阻断，详见 `architecture/runtime.md` §优雅退出）。
...
| 2026-09-18 | graceful-shutdown-hardening | 关停协议重构：后端 SIGTERM 后四步有界关停（① `srv.Shutdown` 5s 摘端口先行 + 排空在途 → ② `Registry.StopAll` 30s → ③ 工作队列停止外包 10s → ④ 单行耗时汇总）；移除 `os.Exit(0)` 使 tracer flush / `logging.Close` defer 真实执行、demo 模式也优雅摘端口；端口由「等进程退出」变 7ms 级释放（`start-dev.sh stop` 不再需 kill -9 兜底） | [`openspec/changes/archive/2026-09-18-graceful-shutdown-hardening`](../../../openspec/changes/archive/2026-09-18-graceful-shutdown-hardening) |
