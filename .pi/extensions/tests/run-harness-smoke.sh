...
# dev-process-guard（孤儿 dev 进程治理：八模式纯函数 + decide 分支表 + handler 工厂注入桩回放 +
# B-12 真实进程版）。
...
npx -y esbuild ../lib/session-rollup.ts --bundle --platform=node --format=cjs \
  --outfile=./.srollup.cjs >/dev/null
npx -y esbuild ../lib/dev-process-scan.ts --bundle --platform=node --format=cjs \
  --outfile=./.dps.cjs >/dev/null
npx -y esbuild ../dev-process-guard.ts --bundle --platform=node --format=cjs \
  --outfile=./.dpg.cjs >/dev/null
