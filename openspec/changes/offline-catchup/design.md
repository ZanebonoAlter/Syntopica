...
**口径修订（review H1，2026-09）**：上述下界从滚动 `now()-N*24h` 改为**本地日历天零点减 N 天**（`time.Date(now 年,月,日,0,0,0,0,本地时区).AddDate(0,0,-N)`），删除条件仍严格 `<`。原因：守卫与补档扫描用的是日历天（`IsDateOutsideRebuildWindow` = `date < NormalizeReportDate(today).AddDate(0,0,-N)`），滚动窗会在边界日 D=today-N 当天上午就把 D 的边删掉，而守卫仍放行重建 D → 空报告覆盖好报告。改后 D 当天任意时刻创建的边都保留，与守卫「date==下界放行」同口径。

**口径修订（review M5-B，2026-09，用户拍板）**：GC 范围限**已归档文章**的边——删边与受影响 tag pluck 的谓词同时加 `article_id IN (SELECT id FROM articles WHERE archived = true)`。未归档文章的边永不回收，归档后才进入 N 天倒计时；理由：活跃文章仍在分析面上（阅读页标签角标/过滤直接消费边），标签是活数据，归档才意味着退出分析面。稳态边表估算随之变为 **活跃文章 × 边密度 + 归档文章 7 天窗**（不再是「全部文章 × 7 天窗」；活跃部分不随窗口收敛，但等于用户当前实际在看的标签量）。
...
- **[极端情形澄清，非缺陷]** 停机恰满 N 天 + 当晚队列未清顺延 1 天 → 最老缺档日恰好在第 N+1 天被扫到时已掉出窗口（该日边已在中间某轮 GC 回收），不可补。这是上面「窗口硬边界」的极端叠加情形，不是新缺陷：窗口语义本就是「边在=可重建」，此情形下边确已不在；需要覆盖就把 `tag_edge_retention_days` 调大。
