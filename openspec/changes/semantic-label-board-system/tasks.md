...
## 13. 手动 Board 辅助标签推荐与 Composition 管理

- [ ] 13.1 后端：新增 suggest-auxiliaries API（board embedding vs 全量辅助标签 cosine similarity 排序 + 分页 + 搜索过滤）
- [ ] 13.2 后端：新增 POST /:id/composition 添加接口（幂等写入 board_composition）
- [ ] 13.3 后端：编写推荐 API 和添加接口的单元测试
- [ ] 13.4 前端：新增可复用组件 AuxiliaryLabelPicker.vue（推荐列表 + 搜索 + 分页 + 勾选）
- [ ] 13.5 前端：改造 AddSemanticBoardDialog.vue（嵌入 AuxiliaryLabelPicker，选中 IDs 随创建请求提交）
- [ ] 13.6 前端：改造 BoardCompositionPanel.vue（增加"添加"按钮，弹出 AuxiliaryLabelPicker，支持手动触发推荐）
- [ ] 13.7 前端：编辑已有 board 时支持手动触发推荐（复用 AuxiliaryLabelPicker，排除已关联标签）
- [ ] 13.8 前端：补充 API client 新接口（suggestAuxiliaries、addComposition）
- [ ] 13.9 验证：后端 go build + go test + golangci-lint
- [ ] 13.10 验证：前端 pnpm lint + nuxi typecheck + pnpm build
