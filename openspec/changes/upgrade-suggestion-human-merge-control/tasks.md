...
- [ ] 6.2 将 `clusterCandidates()` 自聚类核心从 centroid 阈值聚类替换为 `average-link greedy`：候选需至少接近一个真实成员，并且与簇内成员平均距离不超过阈值；多簇满足时选择平均距离最小者
- [ ] 6.3 更新/新增聚类测试：覆盖“最大簇不应吸收大量互不相似标签”、`average-link` 分组稳定性、阈值边界、board_affinity 计算仍基于最终簇
