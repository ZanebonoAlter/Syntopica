...
**Reason**: 本 change 移除单文件 `DATABASE_FIELDS.md`（1433 行 / 99.5KB，超单次 read 上限），字段字典按业务域切分为 `docs/reference/database/tables/` 下 12 个域文档；锚定旧文件名的 Requirement 随之失效。

**Migration**: 准确性契约（引用真实存在的 Go model / 不引用已删除路径）由下方 ADDED 的 `Database table docs SHALL reference existing Go models` 域文档版承接，原两个 Scenario 的断言不变、锚点改为域文档集与入口表清单。
