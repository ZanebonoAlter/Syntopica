...
### Requirement: 旧叙事生成函数活跃使用
旧的 `GenerateAndSaveForBoard`/`GenerateAndSaveForAllBoards` 函数 SHALL 标注为 deprecated，不再被定时任务或手动触发调用。旧 NarrativeBoard/NarrativeSummary 的 API 路由 SHALL 保留（向后兼容，只读），但不再写入新数据。

#### Scenario: 旧 API 向后兼容
- **WHEN** 请求 `GET /api/semantic-boards/:id/narratives?days=7`
- **THEN** 系统 SHALL 仍返回旧的叙事数据（如存在），不做任何修改

#### Scenario: 定时任务切换
- **WHEN** `narrative_summary` 定时任务触发
- **THEN** 系统 SHALL 调用新的日报生成逻辑，不再调用旧的叙事生成函数
