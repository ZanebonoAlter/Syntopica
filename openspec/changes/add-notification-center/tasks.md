...
- [x] V.7 人工：触发一次日报生成验证完成/失败汇总通知——**部分完成**：①队列表每日重置已在生产实测（2026-09-17 log_cleanup 首轮 tag_jobs completed 19153→535，failed 行保留）；②日报生成链路通知待 AI 模型就绪后人工验证（2026-09-18 触发 /api/daily-reports/generate 时模型未连通，无版面产出，通知链路留待下次真实生成观察；互斥/单条口径已被 handler+scheduler 单测四分支覆盖）。留痕：模型就绪后触发一次生成，确认收到通知且至多一条
