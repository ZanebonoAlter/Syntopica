...
用户从 PC 浏览器访问常驻在 Pi 上的 Syntopica。两条产出路径共用同一目标（浏览器只面对一个 origin），共用同一构建期注入：

- **路径 A（单镜像，仓库既有）**：`docker compose up` 建出前后端合一的镜像，后端 `internal/app/static.go` 托管静态产物，`:5100` 单入口。
- **路径 B（反代制品，本次新增）**：后端按原方式裸跑在 `:5100`，Caddy 在 `:80` 提供静态产物并反代四条后端路径。

主链路节拍（以路径 B 为交付主线，路径 A 由 tasks 1.1/1.2 单独保障）：
