...
定义开发期前后端网络访问契约：后端默认端口 5100、前端绝对 apiBase 直连 + CORS 白名单、dev server 全接口绑定与 localhost 探测卫生约定，消除 Windows 5000 端口保留段冲突（svchost 抢占导致 v4 半开）与 v6-only 监听（部分平台 localhost 解析为 ::1，IPv4 环回与跨主机够不着）两类连通性根因。
