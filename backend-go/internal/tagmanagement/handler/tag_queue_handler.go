...
func getTagQueueStatusReader() *tagQueueStatusReader {
	tagQueueStatusOnce.Do(func() {
		tagQueueStatusService = &tagQueueStatusReader{db: repository.Repo.DB()}
	})
...
func RetagTodayArticles(c *gin.Context) {
	...
	if err := repository.Repo.DB().Where("pub_date >= ?", startOfDay).Find(&articles).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
		return
	}
	...
	queue := repository.NewTagJobQueue(repository.Repo.DB())
	enqueued := 0
	...
	for _, article := range articles {
		...
		if err := queue.Enqueue(repository.TagJobRequest{
			ArticleID:    article.ID,
			FeedName:     feed.Title,
			CategoryName: service.FeedCategoryName(feed),
			ForceRetag:   true,
			Reason:       "retag_today",
		}); err != nil {
			continue
