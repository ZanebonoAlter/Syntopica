...
//nolint:unused // retained for potential future re-enable
...
func validateTreeReviewMove(move treeReviewMove, tagMap map[uint]*TreeNode) error {
	...
	maxDepth := getMaxDepthForCategory(node.Tag.Category)
	if childSubtreeDepth+parentAncestryDepth+1 > maxDepth {
		return fmt.Errorf("move %d -> %d would exceed max depth %d", move.TagID, move.ToParent, maxDepth)
	}
	return nil
}
...
func validateTreeReviewMerge(merge treeReviewMerge, tagMap map[uint]*TreeNode) error {
	...
	maxDepth := getMaxDepthForCategory(sourceNode.Tag.Category)
	if childSubtreeDepth+parentAncestryDepth+1 > maxDepth {
		return fmt.Errorf("merge %d -> %d would exceed max depth %d after migration", merge.SourceID, merge.TargetID, maxDepth)
	}
	if err := validateTreeReviewMergeRelationMigrations(database.DB, merge, sourceNode.Tag.Category); err != nil {
		return err
	}
	return nil
}
...
func validateTreeReviewMergeRelationMigrations(db *gorm.DB, merge treeReviewMerge, category string) error {
	...
	for _, rel := range sourceParentRelations {
		...
		if err := checkDepthLimit(db, merge.TargetID, rel.ChildID, category); err != nil {
	...
	for _, rel := range sourceChildRelations {
		...
		if err := checkDepthLimit(db, rel.ParentID, merge.TargetID, category); err != nil {
...
func executeTreeReviewMove(move treeReviewMove) error {
	...
	if err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		maxDepth := getMaxDepthForCategory(parent.Category)
		if childSubtreeDepth+parentAncestryDepth+1 > maxDepth {
			return fmt.Errorf("depth limit: placing subtree(depth=%d) under parent(ancestry=%d) would exceed max depth %d", childSubtreeDepth, parentAncestryDepth, maxDepth)
		}
...
func validateAndCreateReviewAbstract(abs treeReviewAbstract, tagMap map[uint]*TreeNode, category string) (bool, error) {
	...
	// Abstract creation disabled: delegation to PlaceTagInHierarchy takes over
...
//nolint:unused // retained for potential future re-enable of abstract creation in Phase 6
func createAbstractTagDirectly(abstract treeCleanupAbstract, tagMap map[uint]*TreeNode, category string) error {
	slug := Slugify(abstract.Name)
	if slug == "" {
...
func checkNodeLevelAlignment(node *TreeNode, tmpl *CategoryHierarchyTemplate, issues *[]string) {
	...
	if depth > tmpl.MaxLevel {
		*issues = append(*issues, fmt.Sprintf("tag %d(%s) depth %d exceeds max %d",
			node.Tag.ID, node.Tag.Label, depth, tmpl.MaxLevel))
	}
	for _, child := range node.Children {
...
func Phase6_DedupL1(ctx context.Context, tmpl *CategoryHierarchyTemplate) (int, error) {
	...
	if err := database.DB.Where("category = ? AND source = ? AND status = 'active'", tmpl.Category, "abstract").Find(&l1Tags).Error; err != nil {
		return 0, fmt.Errorf("load L1 tags: %w", err)
	}
	merged := 0
	for i := range l1Tags {
		depth := getTagDepthFromRoot(l1Tags[i].ID)
		if depth == 0 {
			dedupAtDepth(ctx, &l1Tags[i], 0)
			var count int64
			database.DB.Model(&models.TopicTag{}).Where("id = ? AND status = 'active'", l1Tags[i].ID).Count(&count)
			if count == 0 {
				merged++
			}
		}
